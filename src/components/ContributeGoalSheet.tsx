...
export function ContributeGoalSheet({ open, onClose, goal, onSaved }: Props) {
  ...
  return (
    <Sheet open={open} onClose={onClose} title={`${goal.emoji} ${goal.name}`}>
      <form onSubmit={submit} className="space-y-3">
        <p className="text-xs text-muted-foreground text-center">
          Faltan <span className="text-primary font-semibold num">${remaining.toLocaleString("es-AR")}</span> para cumplir tu meta
        </p>
        <input autoFocus value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Cuánto sumás $0,00" inputMode="decimal"
        ...
        <p className="text-[11px] text-muted-foreground text-center">Tip: usá montos negativos para retirar.</p>
        <button disabled={busy} className="tap w-full h-12 rounded-xl bg-primary text-primary-foreground font-semibold flex items-center justify-center gap-2 disabled:opacity-60">
          {busy && <Loader2 className="h-4 w-4 animate-spin" />} Sumar al ahorro
        </button>
      </form>
    </Sheet>
  );
